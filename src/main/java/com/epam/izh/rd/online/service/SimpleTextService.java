...
public class SimpleTextService implements TextService {
    ...
    public String toJumpCase(String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            if (i % 2 == 0) {
                stringBuilder.append(Character.toLowerCase(text.charAt(i)));
            } else {
    ...
    public boolean isPalindrome(String string) {
        String reverseString = new StringBuilder(string).reverse().toString().replaceAll(" ","");
        return string.length() > 0 && string.replaceAll(" ","").equalsIgnoreCase(reverseString); //TODO
    }
}
